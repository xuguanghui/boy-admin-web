...
;(function($, window, document, undefined) {
	...
	Tree.prototype = {
		...
		// domain attribute nodeid, which is used to lookup the node in the flattened structure.
		_findNode: function(target) {

			var nodeId = target.closest('li.list-group-item').attr('domain-nodeid'),
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				var treeItem = $(self._template.item)
					...
					.attr('domain-nodeid', node.nodeId)
		...
		// 1. selectedNode
		// 2. node|domain assigned color overrides
