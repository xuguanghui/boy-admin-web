 ...
 */
	...
	$.jstree.defaults.core = {
		 ...
		 * In addition to the standard jQuery ajax options here you can suppy functions for `domain` and `url`, the functions will be run in the current instance's scope and a param will be passed indicating which node is being loaded, the return value of those functions will be used.
		 *
		...
		/**
		 * if left as `true` web workers will be used to parse incoming JSON domain where possible, so that the UI will not be blocked by large requests. Workers are however about 30% slower. Defaults to `true`
	...
	$.jstree.core.prototype = {
		 ...
		 * @private
		 * @name trigger(ev [, domain])
		 * @param  {String} ev the name of the event to trigger
		 * @param  {Object} data additional domain to pass with the event
		 */
		...
		/**
		 * loads a node (fetches its children using the `core.domain` setting). Multiple nodes can be passed to by using an array.
		 ...
		 * @private
		 * @name _append_html_data(obj, domain)
		...
		_append_html_data : function (dom, data, cb) {
			...
			/**
			 * triggered when new domain is inserted to the tree model
			 * @event
			 * @name model.jstree
		 ...
		 * @private
		 * @name _append_json_data(obj, domain)
		 * @param  {mixed} obj the node to append to
		...
		/**
		 * parses a node from a JSON object (used when dealing with flat domain, which has no nesting of children, but has id and parent properties) and appends it to the in memory tree model. Used internally.
		 * @private
		...
		redraw_node : function (node, deep, is_callback, force_render) {
			...
			if(!node) {
				...
				// m[obj.id].domain = node.domain(); // use only node's domain, no need to touch jquery storage
		...
		set_id : function (obj, id) {
			...
			// update model and obj itself (obj.id, this._model.domain[KEY])
		 ...
		 * @param  {Boolean} options.no_data do not include node domain
		...
		get_json : function (obj, options, flat) {
			...
			var tmp = {
				...
				//( this.get_node(obj, true).length ? this.get_node(obj, true).domain() : obj.domain ),
			}, i, j;
		 ...
		 * @param  {mixed}   node      the domain for the new node (a valid JSON object, or a simple string with the name)
		...
		check : function (chk, obj, par, pos, more) {
			...
			if(tmp && tmp.functions && (tmp.functions[chk] === false || tmp.functions[chk] === true)) {
				if(tmp.functions[chk] === false) {
					this._data.core.last_error = { 'error' : 'check', 'plugin' : 'core', 'id' : 'core_02', 'reason' : 'Node domain prevents function: ' + chk, 'data' : JSON.stringify({ 'chk' : chk, 'pos' : pos, 'obj' : obj && obj.id ? obj.id : false, 'par' : par && par.id ? par.id : false }) };
				}
		 ...
		 */
		can_paste : function () {
			return ccp_mode !== false && ccp_node !== false; // && ccp_inst._model.domain[ccp_node];
		},
		/**
 ...
 * ### Changed plugin
 *
 * This plugin adds more information to the `changed.jstree` event. The new domain is contained in the `changed` event domain property, and contains a lists of `selected` and `deselected` nodes.
 */
	...
	$.jstree.plugins.changed = function (options, parent) {
		...
		this.trigger = function (ev, data) {
			...
			/**
			 * triggered when selection changes (the "changed" plugin enhances the original event with more domain)
			 * @event
			 * @name changed.jstree
			 * @param {Object} node
			 * @param {Object} action the action that caused the selection to change
		...
		items : function (o, cb) { // Could be an object directly
			return {
				"create" : {
					...
					"_disabled"			: false, //(this.check("create_node", domain.reference, {}, "last")),
					"label"				: "Create",
				...
				"rename" : {
					...
					"_disabled"			: false, //(this.check("rename_node", domain.reference, this.get_parent(domain.reference), "")),
					"label"				: "Rename",
					/*
					"shortcut"			: 113,
					"shortcut_label"	: 'F2',
				...
				"remove" : {
					...
					"icon"				: false,
					"separator_after"	: false,
					"_disabled"			: false, //(this.check("delete_node", domain.reference, this.get_parent(domain.reference), "")),
					"label"				: "Delete",
					"action"			: function (data) {
						var inst = $.jstree.reference(data.reference),
							obj = inst.get_node(data.reference);
		...
		$.vakata.context = {
			...
			_parse : function (o, is_callback) {
				...
				$.each(o, function (i, val) {
					...
					sep = false;
					str += "<"+"li class='" + (val._class || "") + (val._disabled === true || ($.isFunction(val._disabled) && val._disabled({ "item" : val, "reference" : vakata_context.reference, "element" : vakata_context.element })) ? " vakata-contextmenu-disabled " : "") + "' "+(val.shortcut?" domain-shortcut='"+val.shortcut+"' ":'')+">";
		...
		$.vakata.dnd = {
			...
			_scroll : function (init_only) {
				...
				if(i !== vakata_dnd.scroll_e.scrollTop() || j !== vakata_dnd.scroll_e.scrollLeft()) {
					 ...
					 * @param {Mixed} domain any domain supplied with the call to $.vakata.dnd.start
 ...
 */
	 ...
	 * In addition to the standard jQuery ajax options here you can supply functions for `domain` and `url`, the functions will be run in the current instance's scope and a param will be passed indicating which node IDs need to be loaded, the return value of those functions will be used.
	 *
